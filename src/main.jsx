...
function App() {
  ...
  useEffect(() => {
    if (!supabase || !session?.user?.id || viewMode !== 'lobby') return undefined;
    refreshCommunityFriendInvites();
    const refreshOnFocus = () => refreshCommunityFriendInvites();
    window.addEventListener('focus', refreshOnFocus);
    const interval = window.setInterval(refreshCommunityFriendInvites, 12000);
    return () => {
  ...
  if (viewMode === 'lobby') {
    return <LobbyScreen activeUser={activeUser} friends={friendsList} requests={friendRequests} communityInvites={communityFriendInvites} socialStatus={friendStatus} communityInviteStatus={communityFriendInviteStatus} search={friendSearch} setSearch={setFriendSearch} onSearch={sendFriendRequest} onRespond={respondToFriendRequest} onRespondCommunityInvite={respondToCommunityFriendInvite} onRefreshCommunityInvites={refreshCommunityFriendInvites} onOpenFriend={openDirectChat} communities={communityRailItems} currentCommunityId={communityId} onCommunity={(item) => { setViewMode('community'); switchCommunity(item); }} onCreateCommunity={() => setShowCommunityCreator(true)} />;
  }

  if (viewMode === 'dm') {
    return <DirectChatScreen activeUser={activeUser} friend={selectedFriend} messages={directMessages} communityInvites={communityFriendInvites} draft={draft} setDraft={setDraft} onSubmit={sendDirectMessage} onRespondCommunityInvite={respondToCommunityFriendInvite} onRefreshCommunityInvites={refreshCommunityFriendInvites} onBack={openLobby} communities={communityRailItems} currentCommunityId={communityId} onCommunity={(item) => { setViewMode('community'); switchCommunity(item); }} onCreateCommunity={() => setShowCommunityCreator(true)} />;
  }
...
function LobbyScreen({ activeUser, friends, requests, communityInvites, socialStatus, communityInviteStatus, search, setSearch, onSearch, onRespond, onRespondCommunityInvite, onRefreshCommunityInvites, onOpenFriend, communities, currentCommunityId, onCommunity, onCreateCommunity }) {
